export type Category = 'Celulares' | 'Notebooks' | 'Accesorios' | 'Audio' | 'Auriculares';
export type Brand = 'Apple' | 'Samsung' | 'Sony' | 'JBL' | 'Asus' | 'Xiaomi' | 'Lenovo';

export interface Product {
  id: string;
  name: string;
  description: string;
  detailedDescription: string;
  price: number;
  category: Category;
  brand: Brand;
  imageUrl: string;
  images: string[];
  specs: Record<string, string>;
  inStock: boolean;
  isFeatured?: boolean;
  hasDiscount?: boolean;
  discountPrice?: number;
}

export const products: Product[] = [
  {
    id: 'iphone-15-pro-max',
    name: 'iPhone 15 Pro Max',
    description: 'Chip A17 Pro. Diseño de titanio. Cámara principal de 48 MP.',
    detailedDescription: 'El iPhone 15 Pro Max presenta un diseño de titanio de calidad aeroespacial que lo hace liviano y resistente. Cuenta con el chip A17 Pro para un rendimiento gráfico de otro nivel y un sistema de cámaras revolucionario con zoom óptico de 5x.',
    price: 1350000,
    category: 'Celulares',
    brand: 'Apple',
    imageUrl: '/imagenes/iphone-15-pro-max.jpg',
    images: ['/imagenes/iphone-15-pro-max.jpg'],
    specs: {
      'Pantalla': '6.7" Super Retina XDR',
      'Procesador': 'A17 Pro',
      'Almacenamiento': '256GB / 512GB / 1TB',
      'Cámaras': '48MP + 12MP + 12MP',
    },
    inStock: true,
    isFeatured: true,
  },
  {
    id: 'samsung-galaxy-s24-ultra',
    name: 'Samsung Galaxy S24 Ultra',
    description: 'Inteligencia artificial que cambia las reglas del juego.',
    detailedDescription: 'Galaxy S24 Ultra con el nuevo procesador Snapdragon 8 Gen 3 y una increíble integración con Galaxy AI. Diseño en titanio, pantalla plana y el S-Pen incluido.',
    price: 980000,
    category: 'Celulares',
    brand: 'Samsung',
    imageUrl: '/imagenes/samsung-galaxy-s24-ultra.jpg',
    images: ['/imagenes/samsung-galaxy-s24-ultra.jpg'],
    specs: {
      'Pantalla': '6.8" Dynamic AMOLED 2X',
      'Procesador': 'Snapdragon 8 Gen 3',
      'Batería': '5000 mAh',
      'Cámaras': '200MP + 50MP + 12MP + 10MP',
    },
    inStock: true,
    hasDiscount: true,
    discountPrice: 950000,
    isFeatured: true,
  },
  {
    id: 'xiaomi-redmi-note-13-pro',
    name: 'Xiaomi Redmi Note 13 Pro',
    description: 'Cámara de 200MP y carga rápida de 67W.',
    detailedDescription: 'El Redmi Note 13 Pro ofrece una experiencia premium con su cámara de 200MP, pantalla AMOLED de 120Hz y carga rápida de 67W, todo a un precio accesible.',
    price: 480000,
    category: 'Celulares',
    brand: 'Xiaomi',
    imageUrl: '/imagenes/xiaomi-redmi-note-13-pro.jpg',
    images: ['/imagenes/xiaomi-redmi-note-13-pro.jpg'],
    specs: {
      'Pantalla': '6.67" AMOLED 120Hz',
      'Procesador': 'Snapdragon 7s Gen 2',
      'Batería': '5100 mAh',
      'Cámaras': '200MP + 8MP + 2MP',
    },
    inStock: true,
  },
  {
    id: 'macbook-pro-14-m3',
    name: 'MacBook Pro 14" M3',
    description: 'La laptop más avanzada con la revolucionaria familia de chips M3.',
    detailedDescription: 'Una capacidad asombrosa con los chips M3, M3 Pro y M3 Max. Liquid Retina XDR asombrosa, y batería de hasta 22 horas para potenciar tu ritmo de trabajo.',
    price: 1750000,
    category: 'Notebooks',
    brand: 'Apple',
    imageUrl: '/imagenes/macbook-pro-14-m3.jpg',
    images: ['/imagenes/macbook-pro-14-m3.jpg'],
    specs: {
      'Pantalla': '14.2" Liquid Retina XDR',
      'Chip': 'Apple M3 Pro',
      'RAM': '18GB',
      'Almacenamiento': '512GB SSD',
    },
    inStock: true,
    isFeatured: true,
  },
  {
    id: 'macbook-air-13-m2',
    name: 'MacBook Air 13" M2',
    description: 'Súper ligera. Súper chip.',
    detailedDescription: 'Rediseñada en torno al chip M2 de próxima generación, la MacBook Air es sorprendentemente delgada y ofrece una velocidad y eficiencia energética excepcionales.',
    price: 1100000,
    category: 'Notebooks',
    brand: 'Apple',
    imageUrl: '/imagenes/macbook-air-13-m2.jpg',
    images: ['/imagenes/macbook-air-13-m2.jpg'],
    specs: {
      'Pantalla': '13.6" Liquid Retina',
      'Chip': 'Apple M2',
      'RAM': '8GB',
      'Almacenamiento': '256GB SSD',
    },
    inStock: true,
  },
  {
    id: 'thinkpad-e16-i7',
    name: 'ThinkPad E16 i7',
    description: 'Rendimiento profesional y fiabilidad legendaria.',
    detailedDescription: 'La Lenovo ThinkPad E16 con procesador Intel Core i7 ofrece rendimiento empresarial, teclado excepcional y durabilidad certificada MIL-STD en un formato compacto.',
    price: 890000,
    category: 'Notebooks',
    brand: 'Lenovo',
    imageUrl: '/imagenes/thinkpad-e16-i7.jpg',
    images: ['/imagenes/thinkpad-e16-i7.jpg'],
    specs: {
      'Pantalla': '16" FHD IPS',
      'Procesador': 'Intel Core i7-13700H',
      'RAM': '16GB DDR5',
      'Almacenamiento': '512GB SSD',
    },
    inStock: true,
  },
  {
    id: 'airpods-pro-2da-gen',
    name: 'AirPods Pro (2da gen)',
    description: 'Cancelación activa de ruido el doble de potente.',
    detailedDescription: 'Audio espacial personalizado con seguimiento dinámico de la cabeza y una enorme calidad de sonido gracias al nuevo chip H2. Hasta 6 horas de reproducción.',
    price: 250000,
    category: 'Auriculares',
    brand: 'Apple',
    imageUrl: '/imagenes/airpods-pro-2da-gen.jpg',
    images: ['/imagenes/airpods-pro-2da-gen.jpg'],
    specs: {
      'Chip': 'Apple H2',
      'Conectividad': 'Bluetooth 5.3',
      'Duración': 'Hasta 6h (30h con estuche)',
      'Resistencia': 'IP54 (Polvo y agua)',
    },
    inStock: true,
    hasDiscount: true,
    discountPrice: 199000,
  },
  {
    id: 'sony-wh-1000xm5',
    name: 'Sony WH-1000XM5',
    description: 'La mejor cancelación de ruido de la industria.',
    detailedDescription: 'Auriculares inalámbricos con Noise Cancelling líder del sector, calidad de sonido excepcional y comodidad inigualable.',
    price: 360000,
    category: 'Auriculares',
    brand: 'Sony',
    imageUrl: '/imagenes/sony-wh-1000xm5.jpg',
    images: ['/imagenes/sony-wh-1000xm5.jpg'],
    specs: {
      'Tipo': 'Over-ear',
      'Conectividad': 'Bluetooth 5.2',
      'Batería': '30 hrs',
      'ANC': 'Sí, adaptativo',
    },
    inStock: true,
    isFeatured: true,
  },
  {
    id: 'jbl-tune-770nc',
    name: 'JBL Tune 770NC',
    description: 'Sonido puro JBL con cancelación de ruido adaptativa.',
    detailedDescription: 'Los JBL Tune 770NC ofrecen el legendario sonido JBL con cancelación activa de ruido, hasta 44 horas de batería y un diseño plegable ultra cómodo.',
    price: 115000,
    category: 'Auriculares',
    brand: 'JBL',
    imageUrl: '/imagenes/jbl-tune-770nc.jpg',
    images: ['/imagenes/jbl-tune-770nc.jpg'],
    specs: {
      'Tipo': 'Over-ear',
      'Conectividad': 'Bluetooth 5.3',
      'Batería': '44 hrs',
      'ANC': 'Sí, adaptativo',
    },
    inStock: true,
  },
  {
    id: 'apple-watch-series-9',
    name: 'Apple Watch Series 9',
    description: 'Tu compañero inteligente de muñeca.',
    detailedDescription: 'El Apple Watch Series 9 con chip S9 ofrece una pantalla más brillante, doble toque y funciones de salud avanzadas en un diseño icónico.',
    price: 780000,
    category: 'Accesorios',
    brand: 'Apple',
    imageUrl: '/imagenes/apple-watch-series-9.jpg',
    images: ['/imagenes/apple-watch-series-9.jpg'],
    specs: {
      'Pantalla': 'Always-On Retina LTPO',
      'Chip': 'Apple S9',
      'Resistencia': 'WR50',
      'Sensores': 'SpO2, ECG, Temperatura',
    },
    inStock: true,
  },
  {
    id: 'ipad-pro-11-m2',
    name: 'iPad Pro 11" M2',
    description: 'Rendimiento deslumbrante con el chip M2.',
    detailedDescription: 'El iPad Pro con chip M2 ofrece el máximo rendimiento en un diseño increíblemente portátil, pantalla Liquid Retina de 11 pulgadas y compatibilidad con Apple Pencil.',
    price: 850000,
    category: 'Accesorios',
    brand: 'Apple',
    imageUrl: '/imagenes/ipad-pro-11-m2.jpg',
    images: ['/imagenes/ipad-pro-11-m2.jpg'],
    specs: {
      'Pantalla': '11" Liquid Retina',
      'Chip': 'Apple M2',
      'Almacenamiento': '128GB / 256GB / 512GB',
      'Conectividad': 'Wi-Fi 6E',
    },
    inStock: true,
  },
  {
    id: 'galaxy-watch-6-classic',
    name: 'Galaxy Watch 6 Classic',
    description: 'El smartwatch premium de Samsung con bisel giratorio.',
    detailedDescription: 'El Galaxy Watch 6 Classic combina diseño clásico con tecnología avanzada: bisel giratorio, sensor BioActive y Wear OS con One UI Watch.',
    price: 420000,
    category: 'Accesorios',
    brand: 'Samsung',
    imageUrl: '/imagenes/galaxy-watch-6-classic.jpg',
    images: ['/imagenes/galaxy-watch-6-classic.jpg'],
    specs: {
      'Pantalla': '1.47" Super AMOLED',
      'Procesador': 'Exynos W930',
      'Batería': '425 mAh',
      'Resistencia': '5ATM + IP68',
    },
    inStock: true,
  },
  {
    id: 'jbl-flip-6',
    name: 'Parlante JBL Flip 6',
    description: 'Sonido potente y audaz en cualquier lugar.',
    detailedDescription: 'Tu aventura musical sin igual. El altavoz Bluetooth JBL Flip 6 ofrece un potente sonido JBL Original Pro con notable claridad y sistema de altavoces de 2 vías.',
    price: 130000,
    category: 'Audio',
    brand: 'JBL',
    imageUrl: '/imagenes/jbl-flip-6.jpg',
    images: ['/imagenes/jbl-flip-6.jpg'],
    specs: {
      'Potencia': '20W RMS',
      'Conectividad': 'Bluetooth 5.1',
      'Batería': 'Hasta 12 hrs',
      'Protección': 'IP67 Waterproof',
    },
    inStock: true,
    isFeatured: true,
  },
  {
    id: 'samsung-galaxy-s23',
    name: 'Samsung Galaxy S23',
    description: 'Captura la noche, crea momentos épicos.',
    detailedDescription: 'Teléfono compacto potente con cámaras avanzadas de última generación.',
    price: 850000,
    category: 'Celulares',
    brand: 'Samsung',
    imageUrl: '/imagenes/samsung-galaxy-s23.jpg',
    images: ['/imagenes/samsung-galaxy-s23.jpg'],
    specs: {
      'Pantalla': '6.1" Dynamic AMOLED',
      'Procesador': 'Snapdragon 8 Gen 2',
      'Cámara': '50MP',
      'Batería': '3900 mAh',
    },
    inStock: true,
    hasDiscount: true,
    discountPrice: 700000,
  },
];
